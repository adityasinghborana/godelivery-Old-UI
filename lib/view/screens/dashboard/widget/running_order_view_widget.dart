...
class RunningOrderViewWidget extends StatelessWidget {
  ...
  RunningOrderViewWidget({Key key, @required this.reversOrder})
      : super(key: key);
        ...
        child: Column(children: [
          ...
          ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                String _orderStatus =
                    reversOrder != null ? reversOrder[index].orderStatus : '';
                int _status = 0;

                if (_orderStatus == OrderStatusType.pending.name) {
                  _status = 1;
                } else if (_orderStatus == OrderStatusType.accepted.name ||
                    _orderStatus == OrderStatusType.processing.name ||
                    _orderStatus == OrderStatusType.confirmed.name) {
                  _status = 2;
                } else if (_orderStatus == OrderStatusType.handover.name ||
                    _orderStatus == OrderStatusType.picked_up.name) {
                  _status = 3;
                }
                      ...
                      child: Row(
                          ...
                          children: [
                            Center(
                              child: SizedBox(
                                height:
                                    _orderStatus == OrderStatusType.pending.name
                                        ? 50
                                        : 60,
                                width:
                                    _orderStatus == OrderStatusType.pending.name
